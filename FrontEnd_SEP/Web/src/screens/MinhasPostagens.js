...
import { View, Text, Button, StyleSheet, TouchableOpacity, Alert, Image, FlatList, Modal, TextInput, ScrollView, KeyboardAvoidingView, Platform, Animated } from 'react-native';
...
const MinhasPostagens = ({ route }) => {
  ...
  const fetchMinhasPostagens = async () => {
    try {
      ...
      const response = await authenticatedRequest('get', `http://localhost:3000/api/postagens/${usuarioId}`);
  ...
  const openEditModal = (postagem) => {
    ...
    setImagem(postagem.imagemBase64 ? `data:image/jpeg;base64,${postagem.imagemBase64}` : null); // Atualiza a imagem
  ...
  const handleEditPostagem = async () => {
    try {
      ...
      const response = await authenticatedRequest('put', `http://localhost:3000/api/postagens/${postagemEditando.id}`, updatedPostagem);
      ...
      const errorMessage = error.response ? error.response.data.message : 'Ocorreu um erro ao editar a postagem.';
  ...
  const handleDeletePostagem = async (postId) => {
    try {      
      console.log(postId)
      const response = await authenticatedRequest('delete', `http://localhost:3000/api/postagens/${postId}`); 
      ...
      const errorMessage = error.response ? error.response.data.message : 'Ocorreu um erro ao deletar a postagem.';
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.postagem}>
            <Image source={{ uri: `data:image/jpeg;base64,${item.imagemBase64}` }} style={styles.postImage} />
            <View style={styles.postContent}>
              <Text style={styles.postTitle}>{item.nomeAnimal}</Text>
              <Text style={styles.postInfo}>
                Visto por último em: <Text style={styles.postDetail}>{item.dataUltimaVezVisto}</Text>
              </Text>
              <Text style={styles.postInfo}>
        ...
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer}>
              ...
              {imagem && <Image source={{ uri: imagem }} style={styles.selectedImage} />} {/* Exibe a imagem escolhida */}
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.deleteModalContent}>
            <Text style={styles.deleteModalText}>Tem certeza que deseja excluir esta postagem?</Text>
