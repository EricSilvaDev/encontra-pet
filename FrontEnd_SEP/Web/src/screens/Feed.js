...
import { View, Text, Modal, TextInput, Button, StyleSheet, TouchableOpacity, Alert, Image, FlatList } from 'react-native';
...
const Feed = () => {
  ...
  const carregarPostagens = async () => {
    try {
      const response = await authenticatedRequest('get', 'http://localhost:3000/api/postagens');
  ...
  const carregarUsuarioInfo = async (usuarioId) => {
    try {
      const response = await axios.get(`http://localhost:3000/api/usuarios/${usuarioId}`);
  ...
  const enviarPostagem = async () => {
    ...
    try {
      ...
      const response = await authenticatedRequest('post', 'http://localhost:3000/api/postagens', formData);
      console.log(response);
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {/* Botões */}
            <Button title="Fechar" onPress={() => setIsUserModalVisible(false)} />
          </View>
        </View>
      </Modal>
    </View>
